...
router.get("/webclip/info", (req, res) => {
  ...
  res.json({
    ...
    parameters: {
      ...
      webClipURL:
        "string (optional) - HTML filename in assets/html/ or full URL (e.g., index.html or https://example.com)",
      webClipIcon:
        "string (optional) - Icon filename in assets/icons/ (e.g., swag-apple-icon.png)",
    },
    example: {
...
function generateWebClipProfile({
  profileName,
  organization,
  description,
  identifier,
...
function generateHTMLData(htmlUrl, additionalParams = {}) {
  try {
    ...
    <script>
        ...
        logger.warn("HTML file not found, falling back to default", {
          htmlPath,
        });
    ...
    <script>
        ...
        (function() {
          ...
          const base64Data = Buffer.from(htmlData, "utf8").toString("base64");
          logger.info(
            "Using default HTML as fallback with injected parameters",
            {
              defaultHtmlPath,
              fileSize: htmlData.length,
              base64Length: base64Data.length,
...
function generateIconData(iconUrl) {
  try {
    ...
    if (iconUrl && iconUrl.startsWith("http")) {
      logger.info("Icon URL provided, but local file reading is preferred", {
        iconUrl,
      });
      return "";
    }
    ...
    const defaultIconPath = path.join(
      __dirname,
      "../assets/icons/swag-apple-icon.png"
    );
...
// GET /generator - Serve the client-side MDM generator page
router.get("/generator", (req, res) => {
  logger.info("MDM generator page requested");

  const htmlPath = path.join(__dirname, "../assets/html/mdm-generator.html");

  if (fs.existsSync(htmlPath)) {
    res.sendFile(htmlPath);
  } else {
    logger.error("MDM generator HTML file not found", { htmlPath });
    res.status(404).json({ error: "MDM generator page not found" });
  }
});
